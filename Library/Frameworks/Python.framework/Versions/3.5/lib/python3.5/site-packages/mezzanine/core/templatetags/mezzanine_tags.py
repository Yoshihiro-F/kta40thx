...
from footer.models import Footer, Related

register = template.Library()

@register.inclusion_tag('includes/result1.html', takes_context=True)
def footerview1(context):
    active_dates = Footer.objects.values('active_date')
    active_places = Footer.objects.values('active_place')
    address_places = Footer.objects.values('address_place')
    member_chargess = Footer.objects.values('member_charge')
    contact_tos = Footer.objects.values('contact_to')
    mail_addresss = Footer.objects.values('mail_address')
    member_numberss = Footer.objects.values('member_numbers')

    active_date = active_dates[0]
    active_place = active_places[0]
    address_place = address_places[0]
    contact_to = contact_tos[0]
    mail_address = mail_addresss[0]
    member_numbers = member_numberss[0]
    member_charges = member_chargess[0]

    return {
        'active_date': active_date['active_date'],
        'active_place': active_place['active_place'],
        'address_place': address_place['address_place'],
        'contact_to': contact_to['contact_to'],
        'mail_address': mail_address['mail_address'],
        'member_numbers': member_numbers['member_numbers'],
        'member_charges': member_charges['member_charge'],
    }

@register.inclusion_tag('includes/result2.html', takes_context=True)
def footerview2(context):
    list1 = []
    list2 = []
    lists = []
    related_name = Related.objects.values('link_name')
    related_link = Related.objects.values('link_url')
    for x in related_name:
        list1.append(x['link_name'])
    for y in related_link:
        list2.append(y['link_url'])
    lists = zip(list2, list1)
    return{
        'lists':lists
    }
