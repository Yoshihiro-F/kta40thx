...
def imgfil(value):
    while True:
        imgfile = random.choice(os.listdir(settings.STATIC_ROOT + "/" + value))
        #imgfile = random.choice(value))
        if not imgfile == ".thumbnails":
            direfile = "".join([settings.MEDIA_URL,"uploads/backgrounds/",imgfile])
            return direfile
            break
        else:
            continue
